...
> - **Édition d'une séance (§300)** : le coach peut désormais **éditer une séance générée**. Depuis la planif du nageur, tape une séance puis « Éditer la séance » → l'éditeur catalogue s'ouvre. Les métadonnées du mésocycle (`mesocycle_id`, périodisation) sont **préservées**, donc un rejet ultérieur reste cohérent (il restaure bien la planif d'avant). La section « Étape 3 — Ajustement fin » est maintenant exacte.

*Document maintenu en cohérence avec l'état du code au §300 (2026-05-23). Si tu vois un écart entre cette doc et l'app, c'est l'app qui a raison — préviens.*
