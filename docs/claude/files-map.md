...
| `src/lib/api/strength-warmup.ts` | API routines d'échauffement : `getCommonWarmupRoutine()` (Bloc 1 articulaire, `warmup_common_routine`, mig 00214, §351) + `getActivationRoutine()` (Bloc 3 activation regroupée par seau, `warmup_activation_routine`, mig 00215, §352). Injectés dans `MesocycleInput.commonWarmupRoutine`/`activationRoutine` par l'aperçu ; fallback `[]`/`{}`. | 42 lignes |
