...
| `src/components/strength/assessment/AssessmentScoreField.tsx` | Champ de notation 0-3 d'un axe du bilan physique (§301 T5) — slider + repère chiffré + descripteur du niveau choisi + dépliant 4 niveaux avec photos de référence (fallback gracieux) + rappel de la note précédente | 177 lignes |
...
| `src/lib/strength/bilanProgress.ts` | Logique pure de l'état du fil conducteur (`computeBilanProgress`) — §302 | 43 lignes |
