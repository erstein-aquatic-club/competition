...
| `src/lib/api/strength-mesocycles.ts` | Wrappers API mésocycle (`generateMesocyclePreview`, `applyMesocycle` → RPC avec `p_template_id: null` §305, `revertMesocycle` → RPC, `getMesocycle`/`getActiveMesocycle`/`listMesocycles`) + `getStrokeSignatures`/`getDistanceProfiles` (tables de référence §305) — Chantier D §293, taxonomie nage × distance §305 | 481 lignes |
