...
| `src/lib/strength/adjustmentFactors.ts` | `applyAdjustmentFactors(plan, vol, int)` (TS pur) — post-process d'un `GeneratedMesocycle` : `sets ×vol` (clamp ≥1), `intensityPct1rm ×int` (clamp [0,100]) ; laisse intact pct null/0 (plio/BW) ; throw sur facteur ≤0 ; **§375 : allowlist `role!=='developpement'`** — amorces PAP, mobilité corrective et échauffements exclus du scaling — §338/§375 | 61 lignes |
...
| `src/lib/strength/__tests__/mesocycleEngine.test.ts` | Tests TDD du moteur (suites par fonction + orchestrateur ; bloc `periodize startPhase` §338 ; +3 tests intégration affûtage progressif §375) — §293/§375 | 2827 lignes |
| `src/lib/strength/__tests__/taperSetFactor.test.ts` | 8 tests `taperSetFactor` + `phasePositionFor` (paliers 1/2/3 sem., défensif count>3, index négatif clampé, bornes tableau) — §375 | 57 lignes |
| `src/lib/strength/__tests__/adjustmentFactors.test.ts` | 13 tests `applyAdjustmentFactors` (scaling sets/intensité, clamps, plio/BW, throw ; +4 §375 : allowlist developpement, PAP exclu, mobilité corrective exclue, warmup exclu) — §338/§375 | 171 lignes |
...
| `src/lib/strength/physicalStats.ts` | **Module pur des « Stats physiques » du profil** (§387-389) — `populationComparison(score)` (score percentile → « top X % » à résolution fine, jusqu'à top 0,01 %), `buildPhysicalStatsSummary` (points forts ≥ 70 / axes < 50 + fallbacks relatifs + indice global), `summarizeAttempts` (jsonb essais → 1 ligne, poids du nageur masqué §389), `buildKpiHistoryRows` (historique + Δ comparable §388), `formatKpiValue`/`formatNumber`. Zéro I/O, testé `node:test` (16 tests). | 217 lignes |
...
| `src/lib/strength/oneRmCalibration.ts` | Gate pur `needsOneRmCalibration(input)` de la calibration 1RM inline en séance : vrai uniquement série 1 + prescrit %1RM + métrique `weight_kg` + non poids-de-corps + 1RM manquant (restaure les 4 filtres §297, anti-régression incident §368). Consommé par `WorkoutRunner` (carte série 1 augmentée) — §369 | 23 lignes |
...
| `src/lib/liveffn/parseStartlist.ts` | Parser regex de la liste de départ liveffn (html→nageurs+courses, hooks de classes, noms composés), helpers exportés §364 — §361 | 154 lignes |
| `src/lib/liveffn/parseResults.ts` | Parser regex de la vue Résultats liveffn (html→nageurs+courses: place/phase finale\|série/temps/points/splits, DOM-free, frère de parseStartlist) — §364 | 142 lignes |
| `src/lib/competitions/resultVerdicts.ts` | Verdicts purs résultats : `collapseByEvent` (repli des phases, place finale prioritaire) + `eventVerdict` (record perso / objectif / rang historique, bassin-aware, normalise event_code FFN↔compact) — §364 | 135 lignes |
| `src/components/coach/competition/resultsSynthesis.ts` | `buildResultsSynthesis` pur : totaux (records/podiums/finales A/objectifs) + résultats par nageur (events+verdicts), liés d'abord — §364 | 125 lignes |
| `src/components/coach/competition/CompetitionResultsTab.tsx` | Onglet « Résultats » coach : import snapshot liveffn, en-tête tuiles de stats, cartes par nageur avec badges verdict (record/objectif/finale/rang) + splits dépliables — §364 | 620 lignes |
...
| `src/components/coach/competition/CompetitionDetail.tsx` | Vue détail compétition coach plein écran, 4 onglets : **Nageurs** (sélection participants + recherche + ajout par groupe + bandeau suggestion liveffn), **Paramètres v2** (§363 : 3 sections Infos / Liste de départ / Zone danger + sélecteur bassin 25/50 m persisté en `pool_length` + champ « Lien liveffn Résultats » §364), **Jour J** (`CompetitionStartlistPanel` embarqué), **Résultats** (`CompetitionResultsTab` : import+synthèse résultats club §364) ; clé React Query commune `["startlist", id, url]` (un seul fetch suggestion+Jour J) — §362, §363, §364 | 752 lignes |
| `src/components/coach/competition/SwimmerRaceSheet.tsx` | Bottom sheet détail nageur Jour J (clic d'un nageur LIÉ) : meilleur temps **saison** (depuis 1er sept FFN) + **record perso** (all-time) + dates + tableau d'allures (`PaceMatrixInline` réutilisé) pour l'objectif, bassin des allures = bassin de la compétition (fallback objectif) ; aucun fetch ajouté — §363 | 199 lignes |
| `src/lib/competitions/seasonBest.ts` | `currentSeasonStart` + `bestForEvent` — meilleur temps saison vs record perso (all-time), helpers purs testés — §363 | 28 lignes |
...
| `src/lib/chrono-reducer.ts` | State machine chrono + SET_WAVE_OVERRIDES/SET_WAVE_OVERRIDE_FIELD (§130) + `SET_DEFAULT_DEPARTURE` (intervalle global, graine des vagues §384) | ~360 lignes |
...
| `src/components/chrono/ChronoSetup.tsx` | Phase préparation chrono + WaveConfigCard + preset chips Distance/Splits + section Avancé collapsible + sticky footer ; **corps mobile dédié** (`isMobile`) : Programme compact sans scroll (`CompactPresetRow` une-ligne + strip de chips horizontal), **temps de départ global visible par défaut** (`DepartureInput` mm'ss → `SET_DEFAULT_DEPARTURE`), section Lignes & nageurs dense, bottom-sheet d'ajout, footer compact (sans safe-area, le dock la gère) (§130, §155, §384) | 1402 lignes |
...
| `src/pages/coach/TechTestsScreen.tsx` | Page admin « Essais techniques » (route `tech-tests`) : bac à sable diagnostic appareil réel — toasts, infos appareil/env (build, PWA, safe-area-insets), push de bout en bout, cache/SW, haptique/presse-papier (§385, §386) | 382 lignes |
...
| `src/pages/coach/StrengthPlanningScreen.tsx` | Écran coach « Planif Muscu » (refonte §365) — corps = `<StrengthAttendanceBoard/>` (assiduité) + `<CoachMesocyclesAccordion/>` (mésocycles actifs). L'ancien aperçu hebdo read-only `training_plan_applications` a été retiré. | 47 lignes |
| `src/components/coach/strength/StrengthAttendanceBoard.tsx` | Tableau d'assiduité muscu coach (§365) — toggle période 1/2/4 sem + flèches ◀▶, jauge `terminées/prévues` par semaine ISO + bande 7 pastilles Lun→Dim (terminé/débuté/à faire/déplacée/prévu), tooltips, légende. Câble `getStrengthAttendanceData` + `computeAttendance` + `listActiveMesocyclesWithAthletes`. §381 : l'en-tête de carte navigue vers `/coach/strength-cycle/:athleteId`. | 374 lignes |
| `src/pages/coach/AthleteCycleJourney.tsx` | Page coach plein écran « Parcours du cycle » (§381, route `/coach/strength-cycle/:athleteId`) — hero (objectif, « Semaine X/Y · phase » §358, barre de progression, CTA Ajuster) + timeline verticale par segments de phase (`groupPhaseSegments` + `PHASE_STYLES`) avec jauge assiduité fait/prévu et micro-pastilles jour par semaine (`computeAttendance` sur la fenêtre du méso). Phases lues depuis les `week_type` matérialisés. | 463 lignes |
| `src/lib/strength/cycleJourney.ts` | Helpers **purs** parcours du cycle (§380, testés node:test) — `buildJourneyWeekStarts` (lundis du méso) + `groupPhaseSegments` (segments de phases consécutives, timing past/current/upcoming). Consommés par `AthleteCycleJourney.tsx`. | 85 lignes |
| `src/components/coach/strength/CoachMesocyclesAccordion.tsx` | Accordéon des mésocycles actifs (§365) — single-open, lazy mount du `CoachMesocyclePanel` à l'ouverture. | 142 lignes |
| `src/lib/strength/attendance.ts` | Agrégateur **pur** assiduité muscu (§365, testé node:test) — `computeAttendance` (stats semaine ISO tolérantes au décalage + statut par jour) + helpers `slotDate`/`periodDays`/`derivePeriodWeekStarts`. Prévu = slot_overrides, réalisé = runs completed, débuté = runs in_progress. | 193 lignes |
| `src/lib/api/strength-attendance.ts` | Helper API batché lecture seule (§365) — `getStrengthAttendanceData` (2 select `.in(athlete_id)` sur slot_overrides + runs). | 54 lignes |
| `src/lib/coachNav.ts` | Constante neutre `COACH_SWIMMER_INITIAL_TAB_KEY` (deeplink onglet fiche nageur) — relocalisée depuis l'ex-`CoachActiveMesocyclesSection` (§365). | 2 lignes |
...
| `src/components/profile/PhysicalStatsView.tsx` | Vue « Stats physiques » du profil nageur (§387) — indice physique global, points forts / axes, détail par KPI (mesure brute + barre de score + comparaison population « top X % »). KPIs cliquables → `KpiDetailSheet` (§388). Fetch `getLatestKpiMeasurements`, scoring `rankKpis`, agrégation `physicalStats.ts`. | 387 lignes |
| `src/components/profile/KpiDetailSheet.tsx` | Fiche détail SYNTHÉTIQUE d'un KPI muscu (bottom sheet, §388-389) — score & comparaison population, dernière mesure compacte (essais en 1 ligne via `summarizeAttempts`, sans poids du nageur), mini-historique plafonné avec Δ (`getKpiHistory` + `buildKpiHistoryRows`), protocole replié (`KPI_PROTOCOLS`). | 262 lignes |
| `src/components/profile/physicalStatsUi.tsx` | Bouts d'UI partagés Stats physiques (§388) — `TIER_STYLE`, `CONFIDENCE_BADGE`, `comparisonLabel`, `ScoreBar` (évite un cycle d'import vue ↔ fiche). | 40 lignes |
