...
| `src/lib/api/strength-catalog.ts` | Projection « taggée » de `dim_exercices` (`bucket`/`level`/`contraindication_zones`/`stroke_prehab_affinity`/`corrective_axes` §351/`is_core`/`selection_priority`) → `CatalogExercise[]` consommable par le moteur (§293) + `getExerciseGifs(ids)` (démos KPI §301) | 114 lignes |
| `src/lib/api/strength-warmup.ts` | API routine articulaire commune (Bloc 1, §351) : `getCommonWarmupRoutine()` → `exercise_id` ordonnés de `warmup_common_routine` (mig 00214), fallback `[]`. Injecté dans `MesocycleInput.commonWarmupRoutine` par l'aperçu. | 18 lignes |
...
| `src/lib/strength/warmupLabels.ts` | Helpers purs d'affichage de l'échauffement intelligent (§351) : `warmupSectionLabel(kind)` (« Échauffement articulaire »/« Mobilité corrective »), `correctiveChipLabel(axis, side)` (« Mobilité de hanche · côté gauche », labels FR via `MOBILITY_EVOLUTION_AXES`). TDD `node:test` (5). | 46 lignes |
