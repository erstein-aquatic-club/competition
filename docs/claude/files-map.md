...
| `src/lib/strength/physicalStats.ts` | **Module pur des « Stats physiques » du profil** (§387, §388) — `populationComparison(score)` (score percentile → « top X % » à résolution fine, jusqu'à top 0,01 %), `buildPhysicalStatsSummary` (points forts ≥ 70 / axes < 50 + fallbacks relatifs + indice global), `describeAttempts` (jsonb essais → lignes lisibles §388), `buildKpiHistoryRows` (historique + Δ comparable §388), `formatKpiValue`/`formatNumber`. Zéro I/O, testé `node:test` (16 tests). | 228 lignes |
...
| `src/components/profile/KpiDetailSheet.tsx` | Fiche détail d'un KPI muscu (bottom sheet, §388) — score & comparaison population, dernière mesure + décomposition des essais (`describeAttempts`), historique avec Δ (`getKpiHistory` + `buildKpiHistoryRows`), protocole de mesure (`KPI_PROTOCOLS`). | 268 lignes |
