...
| `src/lib/strength/physicalStats.ts` | **Module pur des « Stats physiques » du profil** (§387) — `populationComparison(score)` (score percentile → « top X % » à résolution fine, jusqu'à top 0,01 %), `buildPhysicalStatsSummary` (points forts ≥ 70 / axes < 50 + fallbacks relatifs + indice global), `formatKpiValue`. Zéro I/O, testé `node:test` (9 tests). | 118 lignes |
...
| `src/components/profile/PhysicalStatsView.tsx` | Vue « Stats physiques » du profil nageur (§387) — indice physique global, points forts / axes, détail par KPI (mesure brute + barre de score + comparaison population « top X % »). Fetch `getLatestKpiMeasurements`, scoring `rankKpis`, agrégation `physicalStats.ts`. | 390 lignes |
