...
| `src/components/coach/strength/StrengthAttendanceBoard.tsx` | Tableau d'assiduité muscu coach (§365) — toggle période 1/2/4 sem + flèches ◀▶, jauge `terminées/prévues` par semaine ISO + bande 7 pastilles Lun→Dim (terminé/débuté/à faire/déplacée/prévu), tooltips, légende. Câble `getStrengthAttendanceData` + `computeAttendance` + `listActiveMesocyclesWithAthletes`. §380 : carte dépliable (single-open) → `AttendanceMesocycleJourney` en lazy mount. | 410 lignes |
| `src/components/coach/strength/AttendanceMesocycleJourney.tsx` | Frise « parcours du cycle » d'un nageur (§380) — position « Semaine X/Y · phase » (`mesocyclePosition` + `week_offset` §358), frise hebdo colorée par phase (`detectPhase`/`PHASE_STYLES`) + segments de phases à venir. Phases lues depuis les `week_type` matérialisés (`getStrengthPlanningWeekOverrides`). | 201 lignes |
| `src/lib/strength/cycleJourney.ts` | Helpers **purs** frise parcours du cycle (§380, testés node:test) — `buildJourneyWeekStarts` (lundis du méso) + `groupPhaseSegments` (segments de phases consécutives, timing past/current/upcoming). | 85 lignes |
