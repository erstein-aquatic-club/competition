...
| `src/components/coach/strength/StrengthAttendanceBoard.tsx` | Tableau d'assiduité muscu coach (§365) — toggle période 1/2/4 sem + flèches ◀▶, jauge `terminées/prévues` par semaine ISO + bande 7 pastilles Lun→Dim (terminé/débuté/à faire/déplacée/prévu), tooltips, légende. Câble `getStrengthAttendanceData` + `computeAttendance` + `listActiveMesocyclesWithAthletes`. | 357 lignes |
