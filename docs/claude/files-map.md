...
| `src/components/strength/assessment/BilanHistorySection.tsx` | Section coach « Historique des bilans » (§347) — `listAssessments` → liste date + badge statut, ligne dépliable montrant le détail **read-only** des scores G/D + notes (via `normalizePhysicalTests`) ; monte `MobilityEvolutionChart`. UI via `/frontend-design`. | 237 lignes |
