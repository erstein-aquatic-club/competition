...
| `src/components/strength/MyPlanMesocycleBanner.tsx` | Bandeau « hero » de Mon plan muscu (§342 V5) — présentationnel : objectif (`formatEventGroupLabel`), Semaine X/Y + barre `role=progressbar`, chip de phase (`shortPhaseLabel`), « Généré le … » (V8), accent de phase. UI via `frontend-design`. | 127 lignes |
