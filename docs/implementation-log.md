...
Plan complet : `docs/plans/2026-04-26-infrastructure-robustness-fixes.md`. Ordre numérique vs chronologique : §174 conduit en parallèle de §170 (audit muscu focus, même date), comble le trou laissé entre §170 et §172 (audit chemin nageur).
...
**Décisions :**
- §174 vs §173 : kept §171 dans les SHA commits (déjà dans les messages + SQL comments + worktree name — immuable). Comble le trou §170→§172. Dans la doc, ce chantier est numéroté §174 (suivant après §173 qui a atterri sur main en parallèle).
...
### Patch correctif §172 — contraste dark mode SlotPill (2026-04-26)

Suite à la livraison initiale, le contraste des Sun/Moon dans les pills était limite en dark mode : `text-foreground/70` donnait du blanc-70 % sur fond `--status-success: hsl(142 55% 50%)` (vert moyennement clair en dark) → ratio sous WCAG AA pour les petits glyphes 8px.

- **`DayCell.tsx slotPillTone`** : refactor pour retourner `{ bg, glyph } | null` au lieu d'une simple chaîne. Mapping cohérent par tone :
  - Success (vert) → `bg-status-success` + `text-white` (le vert reste suffisamment foncé en light comme dark pour que le blanc plein passe)
  - In-progress (gris foncé) → `bg-muted-foreground/30` + `text-foreground` (vrai noir/blanc dynamique)
  - Absent (gris pâle) → `bg-muted-foreground/15` + `text-muted-foreground` (intentionnellement dimmed pour matcher la sémantique)
- **`SlotPill`** : prop `tone: string` → `variant: { bg; glyph } | null`. `strokeWidth` passé de 2.5 à 3 — Lucide rendait le glyphe trop fin à 8px sur retina mobile une fois entouré de la pill 14×14.
- Pas de modif sur la `Dumbbell` orange (déjà saturée + thème-aware via `text-orange-500`, contraste OK).

**Fichiers modifiés :**
- MODIFIED : `src/components/dashboard/DayCell.tsx` (+22 / -9)
...
## §152 — Coach QuickView : mode dépannage pour coaches non-titulaires (2026-04-19)
