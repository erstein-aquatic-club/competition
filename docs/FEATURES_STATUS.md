...
| UX Compétitions coach (détail plein écran 3 onglets + timeline hero) | ✅ | `src/components/coach/competition/CompetitionDetail.tsx`, `CoachCompetitionsScreen.tsx`, `Coach.tsx`, `coachRouteState.ts`, `competitionSelectors.ts`, `suggestParticipants.ts` | §362 — Refonte 3 workflows coach. Détail plein écran 3 onglets `CompetitionDetail` : **Nageurs** (sélection participants + recherche + ajout par groupe + bandeau suggestion liveffn), **Paramètres** (nom/dates/lieu/notes + lien liveffn source unique + suppression), **Jour J** (`CompetitionStartlistPanel` embarqué). Timeline = hero « prochaine compétition » (J-X, lieu, nb nageurs, bouton Jour J) + cartes scannables (liste mixte compétitions/entretiens/fins de cycle, couleur par type) ; tap → détail ; création slim → Paramètres. Tuile hub « Echéances » vivante (prochaine compétition + J-X + deep-link route `competitionId`). Helpers purs `nextCompetition`/`suggestedParticipants`, clé React Query commune `["startlist", id, url]`. Pas de migration ni RLS |
| Jour J : détail nageur (allures + meilleurs temps) | ✅ | `src/components/coach/competition/SwimmerRaceSheet.tsx`, `seasonBest.ts`, `CompetitionStartlist.tsx`, `CompetitionDetail.tsx` | §363 — Clic d'un nageur LIÉ en Jour J → bottom sheet `SwimmerRaceSheet` : meilleur temps **saison** (depuis 1er sept FFN) + **record perso** (all-time) + dates + **tableau d'allures** (`PaceMatrixInline`) pour l'objectif, bassin des allures = bassin de la compétition (fallback objectif) ; aucun fetch ajouté. Onglet Paramètres v2 (3 sections) + sélecteur bassin persisté en `pool_length` (mig 00223). Helpers purs `currentSeasonStart`/`bestForEvent`. Limite : `swimmerSex` `null` → matrice non sexo-spécifique |
