...
- **implementation-log.md**: new `§360` entry — context (coach pastes liveffn startlist per competition), changes (mig 00214, edge proxy `liveffn-startlist`, `src/lib/liveffn/*`, `CompetitionStartlist`), files, tests, decisions (live re-fetch; objectiveHelpers code system; edge=proxy), limits (no birthdate tiebreak yet; weak-network robustness = future).
- **ROADMAP.md**: add the `§360` line + update the top `*Dernière mise à jour*`.
...
git commit -m "docs(§360): liste de départ liveffn par compétition"
```
