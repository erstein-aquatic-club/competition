...
*Dernière mise à jour : §189 — Chrono setup : refonte onglets `team`/`club` (équipe coach = manuels + comptes par défaut) + `computeNextWave` auto-incrémente la vague à chaque ajout (V1, V2, …, capé `maxWaves`). `ManualsTabBody` supprimé (logique inlinée). [§186-§188 réservés pace-v2, docs à venir.] (2026-05-01)*
