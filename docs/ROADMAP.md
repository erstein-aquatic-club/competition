...
*Dernière mise à jour : §190-fix2 livré — card "Ma semaine" nageur réintroduit `api.getSessions` pour la détection des ressentis saisis (le RPC `get_swimmer_sessions` retourne `log_session_id = NULL` de manière inconditionnelle). §187 (slider affinement) designé mais pas livré. (2026-05-02)*
