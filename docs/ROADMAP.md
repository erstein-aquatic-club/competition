...
*Dernière mise à jour : 2026-05-25 — **§308 — Mésocycle muscu : remplacement propre d'un plan en cours (anti-orphelins)**. Audit de robustesse post-§307 : la RPC `apply_strength_mesocycle` ne purgeait jamais les slot/week overrides du plan superseded → re-générer avec un autre jeu de jours (norme depuis le picker §307) laissait des séances orphelines (plan affiché = union ancien+nouveau, `MyPlanTab` lit tous les slots sans filtre mésocycle). Correctif (mig **00201**, replace pur 12-arg) : entre snapshot et matérialisation, DELETE des slot/week overrides de l'athlète **à partir de la date de départ effective** (jours pré-départ déjà entraînés préservés ; revert intact via snapshot). Harness RLS remis au niveau 00200+00201 (était bloqué à 00181) + 4 tests §308 (clean-replace, mid-week, isolation 2-nageurs, revert) TDD RED→GREEN. `npm test` 1353+20, RLS 17/17, tsc 0, build 0. Verdict répétabilité : isolation/déterminisme/revert ✅, re-génération mid-saison corrigée. **Précédent §307 Phase 4** — UI jour-aware (picker jours + date de départ, aperçu badges rôle). Différé : badges `MyPlanTab` (Task 4.3).*
