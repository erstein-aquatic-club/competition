...
*Dernière mise à jour : 2026-05-25 — **§307 Phase 4 — Mésocycle muscu jour-aware (UI génération + aperçu)**. Le moteur (v1.1.0) + la RPC (date de départ réelle, 1re semaine partielle) étant déjà migrés, Phase 4 câble l'UI. `MesocycleGeneration.tsx` (976→1244 l.) : remplacement du compteur séances/sem par un **picker 7 jours** (`WeekdayPicker`, amorce Lun/Jeu ambre vs dev violet, samedi désactivé, légende + avertissement sprint sans jour de dev, a11y `role=checkbox`/`aria-*`) + **Section 06 date de départ** (`StartDatePicker` input natif, 1re semaine partielle). Payload : `{weekdays, startDate}` (drop `sessionsPerWeek`/`startWeekMonday`). `MesocyclePreview.tsx` (1242→1296 l.) : `PendingParams` jour-aware, `MesocycleInput` avec `primerWeekdays`/`sessionsPerWeek=weekdays.length`, `applyMesocycle(…, startDate)`, `SessionCard` affiche **jour + badge rôle** (amorce ambre/dev violet/correctif slate). `npm test` 1353 + 20 verts, tsc 0, build 0 ; pas de `test:rls` (UI pure). **Différé : badges `MyPlanTab` (Task 4.3).***
