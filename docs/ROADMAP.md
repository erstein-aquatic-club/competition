...
*Dernière mise à jour : 2026-05-25 — Chore **Unification du runner de tests** livré — 35 fichiers `*.test.ts(x)` importaient `vitest` → inertes sous `node --test` (0 assertion, faux vert), découvert en revue §305. Livré : `vitest.config.unit.ts` (jsdom, `include: src/**/*.vitest.{ts,tsx}`), 4 fichiers renommés `*.vitest.ts` (hooks DOM / env Vite), 31 fichiers portés `vitest`→`node:test`, `npm test` enchaîne les 2 runners, garde-fou `pretest` (`scripts/check-test-runner.mjs`) qui bloque tout import `vitest` dans `*.test.ts(x)`. node:test 980→1324 (+344 assertions réactivées), vitest 4/20, tsc 0. Tests périmés corrigés sans toucher au code produit (`export-pace-pdf` v1→v2 §186, `useSlotCalendar` trou de midi §95b) — aucun bug produit révélé. Limite : `*.test.ts` hors `tsc`, `*.vitest.ts(x)` type-checkés.*
