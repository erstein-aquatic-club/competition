...
*Dernière mise à jour : §194-vagueB livré (2026-05-08) — Resync push subscription auto + reset banner 60j. NEW helpers purs `shouldRefreshPushSubscription` (cooldown 7j) + `shouldShowPushBanner` (re-proposition 60j) dans `pushHelpers.ts` + 10 tests TDD. NEW `refreshPushSubscription(userId)` dans `push.ts` : silent (jamais prompt), upsert pour rafraîchir `updated_at` (anti-cleanup 90j 00085), DELETE des anciens endpoints (anti-rotation Chrome/Firefox). NEW hook `usePushSubscriptionRefresh` monté dans `PushBridge` (App.tsx) — `useRef` anti-rerun + localStorage cooldown 7j. `PushPermissionBanner` écrit désormais un timestamp au dismiss + ré-affiche après 60j ou pour les dismiss legacy sans timestamp (= migration douce, tous les users dismiss avant ce patch verront le banner réapparaître au prochain boot). 662/663 tests, `npx tsc` clean. Vague A (livrée plus tôt : 278 notifs visibles → 49, -82 %). Vague C (tag SW per-notif, gate focused contextuel) reportée.*
