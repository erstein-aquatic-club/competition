...
*Dernière : 2026-06-10 — **§374 — Quick wins audit transversal (robustesse/perf/UX hors muscu)** : lot issu de l'audit 2026-06-10, exécuté par une équipe de 4 agents parallèles. (1) Hall of Fame : 2 RPC bornées `withTimeout(10 s)` avec dégradation propre par branche (`records.ts`). (2) Edge functions FFN : `AbortSignal.timeout(10 s)` sur les fetch extranat (`_shared/ffn-parser.ts`) + erreurs d'upsert par chunk remontées dans la réponse de `ffn-performances` (`upsert_errors[]`, 207 si échec total) — **à redéployer** (ffn-performances, ffn-sync, import-club-records). (3) `Coach.tsx` : `staleTime` sur 6 queries stables (slots/assignments 5 min, catalogues 30 min, KPIs 15 min). (4) Anti-§113 : 7 DELETE/UPDATE de `swim.ts`/`timesheet.ts` vérifient désormais `.select('id')` + throw si 0 ligne (+8 tests). (5) États UI : skeleton + carte d'erreur sur `CompetitionDetail` (fini le flash « introuvable », +4 vitest), carte d'erreur globale + Réessayer sur `Progress`. tsc 0, lint 0 err, node:test 1667/1667, vitest 80/80. Restent ouverts (audit) : file offline ressenti natation, persistance React Query lecture offline.*
