...
*Dernière mise à jour : §193 livré (2026-05-05) — Objectives ↔ compétitions désormais N:N + multi-select Lier + fix PB. Migration 00155 `objective_competitions` (join table + RLS + backfill 3/3 liens) + suite de fixes critiques post-livraison : (1) embed PostgREST `competitions(name, date)` désambiguïsé via FK explicite `competitions!objectives_competition_id_fkey` (la migration introduisait un 2e chemin FK qui rendait l'embed ambigu → query throw silencieuse → 0 objectif partout), (2) cache React Query versionnée `["athlete-objectives", authUid]` + bypass `getAthleteObjectives()` au profit de `api.getObjectives(authUid)` synchrone, (3) `useAuth.authUid` exposé synchroniquement dans le store Zustand, (4) `computeObjectivePerfRow` utilise `findBestTime` pour bridger les codes événements format compact (`"50NL"`) ↔ FFN (`"50 NL"`) via `EVENT_CODE_TO_NAMES` (le filtre strict ne matchait jamais → PB et delta toujours "—"). Multi-select dans l'onglet Lier (Checkbox au lieu de RadioGroup, mutation séquentielle UPSERT idempotent). §187 (slider affinement) designé mais pas livré.*
