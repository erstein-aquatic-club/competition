...
*Dernière mise à jour : §193 livré — Objectives ↔ compétitions désormais N:N. Migration 00155 `objective_competitions` (join table + RLS + backfill 3/3 liens). API : `Objective.competition_ids: string[]`, `linkObjectiveToCompetition`/`unlinkObjectiveFromCompetition` (UPSERT idempotent / DELETE), `getObjectivesByCompetition` lit via la jointure (fix critique : l'ancienne lecture sur `objectives.competition_id` aurait raté les objectifs liés uniquement via la jointure), `createObjective` rollback en cas d'échec du lien. Front : `InfoMyObjectives` filtre via `competition_ids.includes`, `AddObjectiveSheet` onglet "Lier" relaxé (montre tous les objectifs sauf ceux déjà liés, sous-libellé multi-comp), helper `selectLinkableForCompetition` (5 tests TDD remplaçant les 4 §192). Mutation switch vers `linkObjectiveToCompetition`. La colonne legacy `objectives.competition_id` est conservée mais plus écrite. §187 (slider affinement) designé mais pas livré. (2026-05-04)*
