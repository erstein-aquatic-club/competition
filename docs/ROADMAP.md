...
*Dernière mise à jour : §300 livré (2026-05-23) — **Édition coach séance générée, Part 1 (préservation raw_payload)**. Découverte décisive : la RPC `update_strength_session_atomic` écrit déjà `raw_payload` ; le blocage était purement JS (`updateStrengthSession` forçait `null`). Fix isolé : `reconcileMesocyclePayloads` corrèle les items reconstruits aux items source par `ordre`, préserve le `raw_payload` complet des items édités et impose le `mesocycle_id` de la séance aux items ajoutés (revert cohérent, zéro orphelin) ; `raw_payload` ajouté à `StrengthSessionItem` + porté dans le draft. Ne touche pas les transforms partagés (blast radius minimal). **Part 2 (atteignabilité éditeur : getter par id + deeplink catalogue + bouton « Éditer » + test RLS édit→revert) PLANIFIÉE** — `docs/plans/2026-05-23-coach-edit-mesocycle-part2.md`. tsc clean, npm test vert (+5), build OK ; pas de test:rls (autorisation inchangée, seul le contenu raw_payload change).*
