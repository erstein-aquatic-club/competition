...
*Dernière mise à jour : §194 livré (2026-05-08) — Vague A correctifs centre de notifications. (1) Doublon supprimé : `assignments_create` n'écrit plus manuellement `notifications` + `notification_targets` après chaque INSERT `session_assignments` (le trigger SQL `auto_notify_session_assignment` 00045 le faisait déjà → chaque assignation muscu créait 2 notifs/2 pushs identiques). (2) Migration 00156 ajoute `expires_at` adapté à chaque type sur les 6 triggers `auto_notify_*` (session = scheduled_date+1j, compétition = start_date+2j, slot override = override_date+1j, interview = +30j, swimmer comment = +7j) + backfill `created_at + 14j` sur toutes les notifs sans expires_at. **Effet immédiat prod : 278 notifs visibles → 49 (-82 %)**. Vague B (resync push subscriptions périodique, détection rotation endpoint) et Vague C (tag SW per-notif, gate focused contextuel) reportées. 2 tests régression TDD, `npx tsc` clean, 652/653 (1 fail pré-existant non lié).*
