...
*Dernière mise à jour : §292 livré (partiel, 2026-05-17) — Bilan Muscu Chantier A brique A3 (templates de périodisation) — EN COURS. Livré : migration `00166` table `strength_periodization_templates` (RLS lecture authentifié / écriture coach-admin), types TS, 10 tests RLS. Décision majeure validée coach : vocabulaire de cycles révisé — abandon de « hypertrophie » (littérature + plan réel de F. Wagner), nouveau vocabulaire à 6 cycles `prepa_generale / force_max / puissance / maintien / affutage / pic`, chargement hybride (`force_max`→params force de dim_exercices, `prepa_generale`→endurance, reste générique). RESTE : ré-aligner les 7 templates sur les 6 cycles, config de chargement, validation coach, seed. Cf. `docs/plans/bilan-muscu-chantier-A.md` + `bilan-muscu-cycles-vocabulaire.md`.*
