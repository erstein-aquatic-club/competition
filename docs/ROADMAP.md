...
*Dernière mise à jour : §190-fix livré — card "Ma semaine" nageur bascule sur `get_swimmer_sessions` RPC pour résolution per-swimmer (individual / subgroup / group), correctif §190 initial qui s'appuyait sur la résolution group-level de `useSlotCalendar`. §187 (slider affinement) designé mais pas livré. (2026-05-02)*
