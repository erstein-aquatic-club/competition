...
*Précédente : 2026-06-03 — **§368 — Wizard de calibration 1RM guidé en séance (remplace le gate §297)** : la séance se lance en un tap (plus de `OneRmGate` pré-séance, supprimé). Sur la série 1 d'un exo à calibrer, un wizard guide la calibration : mouvement à vide → retex (douleur/aisance/recharger, branche douleur sécurité) → paliers de chauffe suggérés (`suggestNextLoad` : +2.5/+5/+10 kg ou ancrage ~45 % d'une 1RM connue) → série de travail avec **RIR explicite** → calcul 1RM (`estimateOneRM` Epley+RIR). `onComputed` persiste la 1RM, logge la série comme série 1, douleur→note. Après la série 2, une carte de validation (douleur + ressenti de charge) déclenche, si verdict négatif (douleur OU série trop dure OU « trop lourde » via `isNegativeValidation`), un message « qualité > charge » + bouton « Ajuster ma 1RM (−10 %) » (`adjustOneRmDown`, planché). `firstTimeExercises` (exos jamais réalisés via `getPerformedExerciseIds`) gardé sur `isSuccess` → wizard complet la 1ʳᵉ fois, recalcul court ensuite. Helpers purs : `oneRmCalibration.ts` (suggestion/validation/ajustement), `estimateOneRM(w,r,{rir})` (`prDetection.ts`). Composant `OneRmDiscoveryWizard.tsx` (486 l). Nettoyage : `missing1rmFilter.ts` orphelin + test supprimés, commentaires `OneRmGate` périmés reformulés. Tests : helpers node:test + wizard vitest (13) + glue WorkoutRunner ; suite verte (node:test fail=0, vitest 84), tsc 0. Design `docs/plans/2026-06-03-wizard-calibration-1rm-design.md` / `…-wizard-calibration-1rm.md`.*
