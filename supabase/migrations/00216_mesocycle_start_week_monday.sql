-- 00216_mesocycle_start_week_monday.sql
-- §340 Lot 2 (finding C3) — persiste le lundi de la semaine de départ sur
-- `strength_mesocycles`.
--
-- Avant : `MesocycleAdjust` approximait ce lundi depuis `generated_at`
-- (`getMonday(new Date(generated_at))`) → dérive de fuseau (un timestamp UTC
-- tardif, ex. jeudi 22:30Z = vendredi local Paris, décalait `startMonday` d'une
-- semaine → `weeksRemaining` faux → gate « Aperçu » désactivé à tort en fin de
-- cycle). `p_start_week_monday` est DÉJÀ passé à la RPC, mais n'était pas stocké.
--
-- Rétrocompat : colonne NULLABLE, NON backfillée — les mésos antérieurs à §340
-- gardent NULL et l'UI retombe sur l'approximation `generated_at` (désormais
-- TZ-safe côté client). Les nouveaux mésos / régénérations stockent la valeur
-- exacte.
--
-- Recrée `apply_strength_mesocycle` à l'identique (§328 table rase, §326 sans
-- notification, autorisation inchangée) en ajoutant UNIQUEMENT le stockage de
-- `start_week_monday` à l'INSERT du méso.

ALTER TABLE public.strength_mesocycles
  ADD COLUMN IF NOT EXISTS start_week_monday date;

COMMENT ON COLUMN public.strength_mesocycles.start_week_monday IS
  'Lundi de la semaine 1 du plan (date de depart reelle = ce lundi + jour de la 1re seance). NULL pour les mesos anterieurs a §340 (fallback UI sur generated_at).';
