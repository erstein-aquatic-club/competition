-- 00212_apply_mesocycle_clean_replace_full_week.sql
-- §328 — Table rase : régénérer un plan purge TOUTE la semaine en cours de
-- l'ancien plan (pas seulement à partir de la date de départ).
--
-- §308 préservait les jours PRÉ-DÉPART de la semaine de départ (« jours déjà
-- entraînés »). Effet de bord (retour terrain François, régénération mid-week) :
-- le jour pré-départ de l'ANCIEN plan (un mésocycle différent) survivait → la
-- semaine en cours affichait un mélange ancien/nouveau (ex. lundi de l'ancien
-- plan + mar/jeu/ven du nouveau). Pour le workflow coach (régénérations
-- fréquentes), on veut une table rase nette.
--
-- Cette migration recrée la fonction à l'identique (sans notification, §326) en
-- changeant UNIQUEMENT le DELETE des slot_overrides : on purge toute la fenêtre
-- du plan (`week_start BETWEEN start_monday AND window_end`), sans la garde
-- `>= date de départ`. Le snapshot (pris AVANT le DELETE) reste intact → revert
-- restaure l'état d'avant. Les jours pré-départ ne sont pas ré-écrits (le moteur
-- les saute), ils deviennent simplement vides — cohérent : le plan démarre à sa
-- date de départ. Autorisation et matérialisation inchangées.
