-- 00201_mesocycle_clean_replace.sql
-- §308 — `apply_strength_mesocycle` : remplacement PROPRE d'un plan en cours.
--
-- Problème (révélé par l'audit §307) : la RPC apply ne contenait aucun DELETE.
-- Le supersede (étape 2) ne change que le `status` de la LIGNE mésocycle ; la
-- matérialisation (étapes 5/6) est un UPSERT sur (athlete_id, week_start,
-- day_of_week, time_slot). Donc tout jour-de-semaine que le NOUVEAU plan
-- n'occupe pas mais que l'ANCIEN occupait restait intact, pointant sur l'ancien
-- template (superseded). Comme MyPlanTab lit TOUS les slot overrides de
-- l'athlète sans filtre mésocycle, le plan affiché devenait l'UNION ancien+
-- nouveau → séances orphelines (zombies). Aggravé par §307 : le picker rend les
-- jeux de jours arbitraires (ex. passer de Lun/Mer/Ven à Lun/Jeu laissait Mer/Ven).
--
-- Correctif : entre le snapshot (étape 4) et la matérialisation (étape 5), on
-- purge les slot/week overrides de l'athlète À PARTIR de la date de départ
-- effective. Les jours AVANT `v_effective_start` (1re semaine partielle, déjà
-- entraînés) sont préservés. Le snapshot ayant déjà capturé l'état, le revert
-- reste correct (restaure l'ancien plan). Bornage `[start_monday..window_end]`
-- symétrique avec la fenêtre du snapshot.
--
-- Aucun changement de signature (12-arg de 00200) → CREATE OR REPLACE pur.
-- Aucun DELETE de templates : les `strength_sessions` du plan superseded restent
-- (filet du revert, comme aujourd'hui) ; elles ne sont juste plus référencées.
...
BEGIN
  ...
  -- ── 4bis. §308 — Remplacement propre : purge les slots/weeks de l'athlète
  --   À PARTIR de la date de départ effective, AVANT de matérialiser. Les jours
  --   antérieurs (1re semaine partielle, déjà entraînés) sont préservés. Le
  --   snapshot (étape 4) a déjà capturé l'état pour le revert. Sans ce nettoyage,
  --   re-générer avec un autre jeu de jours laisse des séances orphelines.
