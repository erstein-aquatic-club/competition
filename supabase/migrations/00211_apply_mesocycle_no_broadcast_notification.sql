-- 00211_apply_mesocycle_no_broadcast_notification.sql
-- §326 — Suppression des notifications « nouveau mésocycle ».
--
-- L'ancienne RPC `apply_strength_mesocycle` insérait, en fin de transaction, une
-- notification ciblée sur `notification_targets.target_group_id = <groupe du
-- nageur>` → TOUS les nageurs du groupe la recevaient (et le coach). Retour
-- terrain : « tous les nageurs reçoivent une notif quand je génère un cycle pour
-- un autre nageur ; je ne dois pas avoir ce type de notifications ».
--
-- Décision coach : supprimer purement ces notifications. Cette migration recrée
-- la fonction À L'IDENTIQUE en retirant uniquement le bloc notification (et ses
-- 3 variables locales devenues inutiles). Aucune autre logique ne change : les
-- contrôles d'autorisation (`app_user_id`/`app_user_role`), la matérialisation
-- du plan et le remplacement propre §308 sont strictement préservés.
...
BEGIN
  ...
  -- §308 — Remplacement propre : purge À PARTIR de la date de départ effective.
