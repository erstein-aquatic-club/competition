-- 00181_mesocycle_session_names_readable.sql
-- §296 — Refonte des noms de sessions générées par `apply_strength_mesocycle`.
--
-- Avant : `[Méso 0d1a64e4] S03 J2 · puissance · upper_strength` (illisible)
-- Après : `Force haut + Puissance bas` (lisible, alignement design §296)
--
-- Le préfixe `[Méso XX]` et les coordonnées internes (S/J) sont déplacés
-- dans `description`. Le revert n'est pas affecté — il identifie les
-- templates via `raw_payload->>'mesocycle_id'` (00173 ligne 91-95), pas
-- via le nom.
--
-- Cas couverts :
--   • Primary uniquement (ex. cas mobility-override sécurité)
--   • Primary + complement (cas normal multi-bucket vague C)
--   • Cas dégénéré (no main bucket) → "Mobilité corrective"
--
-- Ce script :
--   1. CREATE OR REPLACE apply_strength_mesocycle avec le nouveau format
--      pour les FUTURES générations.
--   2. Backfill des 20 sessions existantes (le test §293 du user) — on
--      reconstitue primary/complement depuis `strength_session_items.raw_payload`
--      (les items 3-4 ont bucket = primary, item 5 = complement éventuel).
--      L'ancien nom complet est conservé dans `description` pour debug.

BEGIN;

-- ── 1. RPC update : nouveau format de nom ───────────────────────────────────
...
-- ── 2. Backfill existing [Méso ...] sessions with the new readable name ──
-- On reconstitue primary/complement depuis raw_payload des items :
-- on prend les buckets distincts NON-mobility ordonnés par min(ordre).
-- L'ancien nom complet est sauvegardé dans `description`.

WITH session_buckets AS (
  SELECT
    si.session_id,
    si.raw_payload->>'bucket' AS bucket,
    MIN(si.ordre) AS min_order
  FROM strength_session_items si
  WHERE si.raw_payload ? 'mesocycle_id'
    AND si.raw_payload->>'bucket' IS NOT NULL
  GROUP BY si.session_id, si.raw_payload->>'bucket'
),
ordered_main AS (
  SELECT
    sb.session_id,
    array_agg(sb.bucket ORDER BY sb.min_order)
      FILTER (WHERE sb.bucket <> 'mobility') AS main_buckets
...
new_names AS (
  SELECT
    om.session_id,
    CASE
      WHEN om.main_buckets IS NULL
        OR array_length(om.main_buckets, 1) IS NULL
        OR array_length(om.main_buckets, 1) = 0
        THEN 'Mobilité corrective'
      WHEN array_length(om.main_buckets, 1) >= 2 THEN
        (CASE om.main_buckets[1]
          WHEN 'upper_strength' THEN 'Force haut'
          WHEN 'lower_strength' THEN 'Force bas'
          WHEN 'upper_power'    THEN 'Puissance haut'
          WHEN 'lower_power'    THEN 'Puissance bas'
          ELSE om.main_buckets[1]
        END) || ' + ' || (CASE om.main_buckets[2]
          WHEN 'upper_strength' THEN 'Force haut'
          WHEN 'lower_strength' THEN 'Force bas'
          WHEN 'upper_power'    THEN 'Puissance haut'
          WHEN 'lower_power'    THEN 'Puissance bas'
          ELSE om.main_buckets[2]
        END)
      ELSE
        (CASE om.main_buckets[1]
          WHEN 'upper_strength' THEN 'Force haut'
        ...
        END)
    END AS new_name
  FROM ordered_main om
)
UPDATE strength_sessions ss
   SET name = nn.new_name,
       description = ss.name  -- conserve l'ancien nom complet pour debug
  FROM new_names nn
 WHERE ss.id = nn.session_id
   AND ss.name LIKE '[Méso %';
