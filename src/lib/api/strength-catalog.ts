...
interface DbRow {
  ...
  stroke_main_affinity: string[] | null;
...
export async function listCatalogExercisesTagged(): Promise<CatalogExercise[]> {
  ...
  const data = assertSupabase(
    await supabase
      .from('dim_exercices')
      .select(
        'id, nom_exercice, bucket, level, contraindication_zones, stroke_prehab_affinity, stroke_main_affinity, corrective_axes, supports_unilateral, is_core, selection_priority, illustration_gif,' +
