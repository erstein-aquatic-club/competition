...
export function EventProgressionSheet({
  ...
  // Fetch all-time performances for main pool (only while sheet is open)
  ...
  const title = eventLabel(eventCode);
  const subtitle = athleteName
    ? `${athleteName} · ${eventPerfs.length} perf${eventPerfs.length !== 1 ? "s" : ""} en ${poolLength}m`
