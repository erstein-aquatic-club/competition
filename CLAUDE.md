...
Dernier § livré : **§336** — **Récap muscu « Wrapped » (stories plein écran, nageur + coach)** : bouton discret en haut à droite de « Mon plan » muscu (et dans `CoachSwimmerFullView`) → overlay façon *Spotify Wrapped* (pages qui défilent au tap/timer). Slides : objectif du plan, forces & axes (KPI du bilan **sans valeurs brutes**, situés vs population par bande de percentile), top 3 progressions des 90 j (Δ% 1RM estimé), tonnage & stats fun. **Zéro migration/endpoint/table** — module pur `src/lib/strength/wrappedStats.ts` (`node:test`) + hook `useStrengthWrapped(athleteId, {active})` (les 2 requêtes lourdes `getStrengthHistory`/`getExercises` gatées derrière l'ouverture du récap, `staleTime` aligné voisins) + overlay `StrengthWrappedRecap` (moteur de stories : autoplay 6 s, tap/hold/swipe, reduced-motion, focus a11y, `key={index}` rejoue les animations ; UI via skill `frontend-design`). Confidentialité : aucune valeur brute KPI/poids athlète côté nageur (`body-weight-coach-only`). Bouton conditionnel (`hasEnoughWrappedData`). node:test 1418, vitest 34, tsc 0, build 0, **pas de `test:rls`**. *(§335 garantie « minimum haut du corps » quand le focus monopolise un segment ; §334 trigger inscription persiste le sexe + débloque méso Ines mig 00215 ; §333 mode coach secondaire badge état de forme.)*
