...
Dernier § livré : **§355** — **Comble 2 trous de seed correctif (rotation thoracique + scapulaire unilatéral)** : vérif terrain (déficit scapulaire D + rotation de torse) → l'axe `t_spine` n'avait que Cat-Cow (dans la routine commune → dédupliqué) et les exos `scapula_control` étaient tous bilatéraux (raffinement unilatéral §352 inopérant). Fix data (mig **00218**, MCP, validé coach) : 2 nouveaux exos d'échauffement **par-côté** (`exercise_type='warmup'`, bucket mobility) — **id 99** « Rotation thoracique (open book) » (`t_spine`, `supports_unilateral`) + **id 100** « Rowing scapulaire unilatéral (élastique) » (`scapula_control`, `supports_unilateral`). Aucun code moteur modifié (`selectCorrectiveWarmup` lit déjà les tags) → pas de test/build. **CHANTIER (8) « échauffement intelligent » CLOS** (génération Blocs 1+2+3 + marquage coach/nageur + édition coach + seeds correctifs complétés). Reliquat optionnel : édition per-séance des warmups + édition in-app des tags Bloc 2. *(§354 écrans édition routines ; §353 marquage vue exécution nageur ; §352 Bloc 3 activation + unilatéral + Raise ; §351 Blocs 1+2 G/D.)*
