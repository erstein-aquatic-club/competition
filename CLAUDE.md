...
Dernière entrée en date : §190-ui (SwimmerHome — "Ma semaine" remplace "Aujourd'hui" sous Bien-être. Suppression du bloc JSX Section C "Aujourd'hui" qui affichait des cards par session du jour avec badges Fait/À faire/Lancer + état "Jour de repos". `SwimmerWeekMatrixCard` déplacée à sa place : juste sous Section B (Bien-être) en première position après le greeting. La vue détaillée `SwimmerWeekSlots` reste en Section G en bas de page. Helpers exportés `buildTodaySessionCompletionLookup` + `isTodaySessionLogged` conservés (utilisés par `SwimmerHome.test.ts`). useMemos + queries de la Section C préservés (cache priming `["sessions", userId ?? user]` partagé avec la matrice). `SwimmerHome.tsx` ~770 → 673 LOC. 19 tests inchangés. `npx tsc` clean.)
