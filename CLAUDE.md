...
Dernier § livré : **§354** — **Écrans coach d'édition des routines d'échauffement** : nouvel onglet « Échauffement » dans `StrengthCatalog` (`WarmupRoutinesEditor`) pour éditer la routine articulaire commune (Bloc 1) + l'activation par seau (Bloc 3) — listes ↑↓ / ajouter (sélecteur catalogue) / retirer + « Enregistrer » par section. Persistance via 2 RPC atomiques `SECURITY INVOKER` `set_warmup_common_routine`/`set_warmup_activation_routine` (mig **00217** ; RLS coach/admin enforce sur l'INSERT → athlète refusé). API `setCommonWarmupRoutine`/`setActivationRoutine`. S'applique aux prochains mésocycles générés. node:test 1548, vitest 71, tsc 0, lint 0, build OK, RLS 5/5 nouveaux. **CHANTIER (8) « échauffement intelligent » CLOS** : génération (Blocs 1+2+3) + marquage (coach aperçu + nageur exécution) + édition coach des routines. Reliquat optionnel : édition per-séance des warmups + tags Bloc 2 (`corrective_axes`/`supports_unilateral`). *(§353 marquage vue exécution nageur ; §352 Bloc 3 activation + unilatéral + Raise, fondé recherche ; §351 échauffement Blocs 1+2 G/D ; §350 infra eslint ; §349 polish V7/UX1-3.)*
