...
Dernière entrée en date : §193 (Objectives ↔ compétitions désormais en N:N. Migration 00155 NEW table `objective_competitions(objective_id UUID, competition_id UUID, created_at, PK composite)` avec FK ON DELETE CASCADE des deux côtés + RLS aligné sur les policies existantes de `objectives` (nageur ses propres lignes, coach/admin tout, committee SELECT) + backfill idempotent depuis `objectives.competition_id` (3/3 liens reconstruits). La colonne legacy `objectives.competition_id` est conservée pour back-compat mais plus écrite par les nouveaux flux. API : `Objective.competition_ids: string[]` (toujours présent), `getObjectives` ajoute `objective_competitions(competition_id)` au select Supabase + mapping array, `getObjectivesByCompetition` réécrit pour lire via la jointure (fix critique : aurait raté les nouveaux liens N:N), `createObjective` INSERT objective puis UPSERT lien avec rollback du row si lien échoue, `updateObjective` ignore désormais `competition_id` (no-op gracieux). NEW `linkObjectiveToCompetition` (UPSERT idempotent ON CONFLICT DO NOTHING) + `unlinkObjectiveFromCompetition` (DELETE), exposés via index.ts + façade api.ts. Front : `selectLinkableObjectives` (§192) supprimé → remplacé par `selectLinkableForCompetition(objectives, currentCompetitionId)` (5 tests TDD). `InfoMyObjectives` filtre via `competition_ids.includes`. `AddObjectiveSheet` onglet "Lier" relaxé — montre TOUS les objectifs sauf ceux déjà liés à cette compet, mutation switch vers `linkObjectiveToCompetition`, NEW query `["competitions"]` + `competitionNameById` Map pour afficher "Déjà lié à : Champ. France, Inter-clubs" sous chaque ligne. Coach `SwimmerObjectivesTab.tsx` non touché (form edit utilise encore la colonne legacy mono-valeur, out-of-scope). 4 commits sur main, 651 tests, 650 verts, 1 fail pré-existant `transformers.test.ts`. `npx tsc` clean.)
