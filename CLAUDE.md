...
Dernière entrée en date : §190-fix3 (Card "Ma semaine" — exclure les séances muscu. Ajout du filtre `row.slot_session_type !== "swim"` dans la boucle d'indexation `byDateBucket`. Les rows strength sont ignorées dès l'indexation, ne comptent ni dans `totalSlots` ni dans `plannedPast/donePast/missedCount`. La muscu reste visible via `MyPlanWeekCard` (côté Strength) et la Section "Aujourd'hui" du SwimmerHome. 16 tests inchangés. `npx tsc` clean.)
