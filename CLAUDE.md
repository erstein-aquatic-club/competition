...
Dernière entrée en date : §172 (Audit robustesse chemin nageur — calendrier, focus, plan→drawer : nouveau hook `useStrengthPlanByISO` qui fusionne `strength_planning_slots` groupe + overrides per-athlète via `mergeStrengthSlots` §157 et expose `strengthByISO`/`resolvedByISO` au calendrier nageur. DayCell : icône Dumbbell en haut-gauche si muscu prévue (Trophy compétition prioritaire) + pills AM/PM 14×14 avec mini Sun/Moon dedans (couleur de fond préservée pour scan statut). FeedbackDrawer : carte muscu dédiée du jour (handoff sessionStorage `eac_pending_strength_focus_slot_id` → /strength). MyPlanWeekCard : bouton CTA "Démarrer maintenant" sur la séance jour-J semaine courante, court-circuite le reader via `autoLaunchKey` mécanisme dans Strength.tsx. WorkoutRunner : mode tunnel numpad charge→reps + indicateur 1/2→2/2, confirmation skip exercice si logs>0, boutons difficulté h-9, safe-area-inset-top sur exit bar. FeedbackDrawer : hint permanent "Remplis les 4 indicateurs", boutons ressenti h-11/h-12. Auth `auth.ts` : tolérance 3 échecs refresh consécutifs avant signOut (fini le logout brutal pendant un set). Strength.tsx : `setIsFinishing(false)` dans catch onFinish, invalidations `["assignments"]` par préfixe (couvre Dashboard + Strength), `wasOfflineRef` reset après succès reconcile. Dashboard.tsx : fermeture session différée à `mutation.onSuccess`. Bug TZ latent corrigé via TDD (`buildWeekStarts` shiftait à UTC en CEST). Tests +8 sur le hook, 253/253 (vs 247 baseline, 0 régression).
