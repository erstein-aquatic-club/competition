...
Dernier § livré : **§333** — **Mode coach secondaire : plus de détail sur l'état de forme** : `SwimmerFormBadge` (section Forme de `CoachSwimmerQuickView`) corrigé — `readiness_score` désormais lu sur 0-100 (était traité /10 → toujours « Bonne », sparkline cassée) avec bandes alignées sur le reste de l'app + affichage `%` ; **ajout** d'une grille des 5 sous-métriques moyennées 7j (sommeil/fatigue/courbatures/humeur/stress, /5, colorées), heures de sommeil moyennes et dernière note bien-être. Composant utilisé uniquement par le QuickView → portée = mode coach secondaire. `tsc` 0, node:test 1401, vitest 24, build 0. Pas de migration/RLS. *(§332 repos cycles dérivés borné + staples allégés mig 00214 ; §331 badge durée estimée ; §330 auto-réparation cache PWA.)*
