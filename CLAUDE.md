...
Dernière entrée en date : §175 (Consolidation post-audit nageur — 4 P2 résiduels + tests régression : `Dashboard.tsx authUuid` réactif via `onAuthStateChange` au lieu de capture unique au mount. `Strength.tsx` pré-persistance synchrone localStorage du focus-state avant `setActiveRunId` — anti-orphelin si app killée entre `startRun.mutateAsync` succès et React commit. `OfflineMutationSync` toast batched "Données obsolètes ignorées" sur types non reconnus avec liste des types affectés (au lieu de drop silencieux post-downgrade PWA). Nouvel event `QUEUE_REAPED_EVENT` dispatché par `getQueue` quand items reaped TTL/poison, écouté par `OfflineMutationSync` pour toast destructive "Données hors-ligne abandonnées" avec contexte. NEW `src/lib/api/__tests__/strength.test.ts` (8 tests via `node:test + mock.module`) couvrant régression §159 sur `updateStrengthRun` (assignment update fail = throw, pas swallow) + branches `reconcileStrengthRunLogs`. NEW `src/components/dashboard/__tests__/DayCell.test.tsx` (7 tests SSR `renderToStaticMarkup`) sur priorité Dumbbell/Trophy + contrats glyphe SlotPill par variant. 340 → 355 tests (+15). C4.2/C4.4/C4.5 reportés §176 (RTL non installé). RLS Phase 3 bloqué Docker non démarré.
