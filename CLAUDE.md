...
Dernier § livré : **§346** — **Bilan muscu : mobilité gauche/droite + notes (Slice A : data + moteur + saisie)** : forme uniforme `{left,right,note?}` par axe dans le jsonb `physical_tests` (**aucune migration**) ; module pur `physicalTests.ts` (`normalizePhysicalTests` upcaste l'ancien number, `effectiveAxisScore=min`) ; moteur `scoreMobility`/`dysfunctionFlags` consomme `min(G,D)` (asymétrie unilatérale = dysfonction, rétrocompat) ; UI (`/frontend-design`) `AssessmentBilateralField` (G|D + note repliable) + note de synthèse globale ; axes G/D shoulder/t_spine/hip/scapula/hip_hinge, `trunk_neck_alignment` unique. TDD ; tsc 0, node:test 1498, vitest 60, build OK. **Slice B (§347) à venir** : historique bilans + courbe d'évolution. Backlog : (8) [FUTUR] routines d'échauffement pilotées par déficits mobilité. *(§345 polish UI mésocycle + fix bilan coach ; §344 audit flux clôturé ; §343 finitions moteur/runner/a11y.)*
