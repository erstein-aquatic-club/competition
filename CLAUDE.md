...
Dernier § livré : **§334** — **Trigger d'inscription persiste enfin le sexe + débloque méso Ines** : `handle_new_auth_user` n'extrayait pas `sex` du `raw_user_meta_data` (alors que `Login.tsx` le requiert et l'envoie) → tout nouveau compte landait `user_profiles.sex=NULL`, écran `ProfileIncompleteScreen` au lancement du moteur (barèmes KPI sexués). Mig **00215** ajoute extraction défensive (domaine `{'M','F'}` sinon NULL pour déclencher proprement le gate plutôt que corrompre le lookup) ; data : Ines `sex='F'`, `body_weight=80` (estimation coach). Audit codebase = `body_weight` jamais athlete-facing (mémoire `body-weight-coach-only` posée). Bilan KPI Ines transmis (force-puissance d'élite p90+ sauf weighted_pullup ≈p30 → levier #1 pour 50 m crawl). *(§333 mode coach secondaire badge état de forme `SwimmerFormBadge` ; §332 cohérence durées dérivées ; §331 badge durée estimée.)*
