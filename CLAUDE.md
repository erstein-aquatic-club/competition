...
Dernier § livré : **§345** — **Retour terrain François : polish UI mésocycle + fix routing bilan coach** : (1) bandeau « Mon plan » condensé 2 lignes + bouton **Récap intégré** (~−50 % hauteur) ; (2) overflow titres séances (`MyPlanSessionSheet` truncate) ; (3) overflow date pivot coach (`input[type=date]` iOS `min-w-0`) ; (4) presets charge clairs (« Alléger / Standard / Augmenter ») ; (5) **bug** « Refaire le bilan » coach routait vers `/strength/questionnaire` (message nageur incohérent) → `/coach/strength-assessment/:athleteId`. tsc 0, node:test 1491, vitest 57, build OK ; aucune migration. **Backlog terrain** (à concevoir) : (6) historique bilans + initier depuis vue Bilan ; (7) mobilité G/D + notes (change le modèle de données) ; (8) [FUTUR] routines d'échauffement pilotées par déficits mobilité. *(§344 filet anti-régression — audit clôturé ; §343 finitions moteur/runner/a11y ; §342 guidage nageur ; §341 source unique phase/semaine + mig 00216.)*
