...
Dernier § livré : **§358** — **Bannière : report de la progression globale après ajustement** : un méso ajusté (§338) repartait à « Semaine 1/N » au pivot ; désormais la bannière « Mon plan » affiche la progression GLOBALE (« Semaine 3/6 »). Colonne `strength_mesocycles.week_offset` (défaut 0, mig **00220** ; backfill méso de François =2) posée à l'ajustement (`MesocycleAdjust` → `weekOffset = phaseInfo.weekIndex` → `setMesocycleWeekOffset` post-apply dans `MesocyclePreview.onSuccess`) ; `mesocyclePosition(start, total, current, weekOffset=0)` globalise (offset>0 = continuation, jamais « Commence bientôt ») ; `MyPlanTab` passe `week_offset`. node:test 1553, vitest 71, tsc 0, lint 0, build OK ; pas de `test:rls` (UPDATE sous policy existante). **Précédent §357 — « Mettre à jour l'app » purge le cache même sur erreur** : ajout `clearAllCaches()` dans le `catch` de `handleCheckUpdate`. tsc 0, lint 0. *(§356 Raise rameur + correctif flexion d'épaule ; §355 trous correctifs t_spine/scapula ; §354 écrans édition routines ; §353 marquage vue exécution nageur.)* — Précédent **§356** — **Raise = rameur ergomètre + correctif flexion d'épaule dédié** : retour terrain (matériel rameur + élastique). (1) Le « Raise » du Bloc 1 (exo 97) passe de corde/montées de genoux au **rameur ergomètre (~3 min)**. (2) Comble le 3ᵉ trou de dédup : `shoulder_flexion` n'avait que Y-T-W + Shoulder Dislocates, tous deux dans la routine commune → dédupliqués → ajout (mig **00219**) d'un correctif dédié **id 101** « Mobilité flexion d'épaule (overhead élastique / wall slide) » (unilatéral, hors routine commune, contre-indiqué si douleur d'épaule). Désormais **tous les axes mobilité ont ≥1 correctif non dédupliqué**. Data-only, aucun code moteur → pas de test/build ; vérifié en base. **CHANTIER (8) « échauffement intelligent » CLOS** (génération Blocs 1+2+3 + marquage coach/nageur + édition coach + seeds correctifs complétés §355/§356). Reliquat optionnel : édition per-séance des warmups + édition in-app des tags Bloc 2 ; pas d'échauffement « élite tier » dédié. *(§355 trous correctifs t_spine/scapula unilatéral ; §354 écrans édition routines ; §353 marquage vue exécution nageur ; §352 Bloc 3 activation + Raise ; §351 Blocs 1+2 G/D.)*
