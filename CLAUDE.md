...
Dernier § livré : **§347** — **Bilan muscu : historique des bilans + courbe d'évolution G/D (Slice B)** : helper pur `mobilityEvolution.ts` (`buildMobilityEvolution` → série chrono `{date,left,right,effective}` par axe) ; UI (`/frontend-design`) `BilanHistorySection` (carte « Historique » = `listAssessments` → date + badge statut + détail read-only G/D dépliable) + `MobilityEvolutionChart` (recharts, sélecteur axe + mode G&D/G/D/côté faible, ≥2 bilans), montés dans `StrengthAssessmentScreen`, query invalidée à create/submit. TDD ; tsc 0, node:test 1501, vitest 64, build OK ; aucune migration. **Chantier bilan mobilité G/D (6)+(7) clos** ; reste (8) [FUTUR] routines d'échauffement pilotées par déficits mobilité. *(§346 mobilité G/D Slice A ; §345 polish UI mésocycle + fix bilan coach ; §344 audit flux clôturé.)*
